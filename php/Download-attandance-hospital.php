...
if(isset($_POST['v-a-section-btn']))
{
	...
	$markedsql = "select mark from Attandance where date='$date' and hospital='$hname';";
	...
	else
	{
		...
		$sql = "select * from Attandance where date='$date' and hospital='$hname' and time='9:00-10:00 Am' order by roll;";
		...
		echo "<table border='2' class='table table-responsive-md table-striped' name='section>$sname>date>$date>Attandance>List'>
			...
			else if($_SESSION['user_type']==='client')
			{
				...
				echo "class='btn btn-info'>back</a><a href='#' class='export btn btn-info'>Download</a><center>STUDENT'S ATTANDANCE LIST ON <font color='red'>$date</font></center></th>
			</tr>
			<tr>
				<td><b>SerialNo</b></td>
